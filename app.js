...
function renderPensionFromStocks(accountName) {
  ...
  // 종목 수에 맞춰 세로 막대그래프 높이를 늘려줌 — 종목이 많아도 이름이 겹치지 않도록
  chartWrap.style.height = Math.max(180, sorted.length * 54 + 40) + "px";
  ...
  state.charts.pension = new Chart($("#pensionChart"), {
    ...
    data: {
      labels,
      datasets: [
        { label: "원금", data: buyData, backgroundColor: "#c9ced9", borderRadius: 4, barThickness: 16 },
        { label: "평가금액", data: valueData, backgroundColor: "#3a6fb0", borderRadius: 4, barThickness: 16 },
      ],
    },
    options: {
      indexAxis: "y",
      ...
      scales: {
        x: { grid: { color: "#e2e5eb" }, ticks: { color: "#6b7280", callback: (v) => maskShort(v) } },
        y: { grid: { display: false }, ticks: { color: "#1a1d24", font: { size: 11.5, weight: "600" } } },
      },
    },
  });
